    ...
    <div>
      {items.map((item, i) => (
        <div key={item.q}>
          ...
          >
            <span className="h-2xs" style={{ color: open === i ? "var(--bone-white)" : "var(--ash-gray)" }}>{item.q}</span>
            <span aria-hidden className="faq-plus" style={{ flexShrink: 0, color: "var(--iris)", fontSize: 26, lineHeight: 1, transform: open === i ? "rotate(45deg)" : "none" }}>+</span>
          </button>
